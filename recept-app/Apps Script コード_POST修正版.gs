// 領収書発行システム Apps Script コード（POST修正版）
...
function doGet(e) {
  try {
    Logger.log('=== doGet開始 ===');
    Logger.log('Request method detected as GET');
    Logger.log('Parameters: ' + JSON.stringify(e.parameter));
    
    // もしPOSTパラメータがあれば、doPostロジックに転送
    if (e.parameter && e.parameter.action === 'post') {
      Logger.log('POST action detected, redirecting to doPost logic');
      return handlePost(e);
    }
...
function handlePost(e) {
  try {
    ...
    Logger.log('トークン検証成功: ' + JSON.stringify(payload));
    ...
    Logger.log('注文データ発見: ' + JSON.stringify(orderRow));
    ...
    if (!receiptNo) {
      ...
      Logger.log('新規領収書番号: ' + receiptNo);
    } else {
      // 再発行
      isReissue = true;
      Logger.log('再発行: ' + receiptNo);
    }
    
    // 仮の成功レスポンス（PDF生成スキップ）
    const file = { getDownloadUrl: () => 'テスト用URL' };
    Logger.log('PDF生成スキップ（テスト用）');
    ...
    Logger.log('台帳更新完了');
    Logger.log('処理完了: ' + receiptNo);
    
    // 成功レスポンス
    Logger.log('JSON応答準備中...');
    ...
    Logger.log('JSON応答: ' + jsonResponse);
    ...
    const errorResponse = JSON.stringify({
      success: false,
      error: '詳細エラー: ' + error.toString() + ' | スタック: ' + (error.stack || 'スタック情報なし')
    });
    
    Logger.log('エラー応答: ' + errorResponse);
