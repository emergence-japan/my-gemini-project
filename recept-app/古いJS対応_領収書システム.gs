// 領収書発行システム - 古いJavaScript対応版
function doGet(e) {
  // フォーム送信の場合
  if (e.parameter && e.parameter.bill_to) {
    var billTo = e.parameter.bill_to;
    var email = e.parameter.customer_email || '';
    
    var emailRow = '';
    if (email) {
      emailRow = '<p><strong>メール:</strong> ' + email + '</p>';
    }
      ...
      '<div class="container">' +
        ...
        '<form method="GET">' +
          '<div class="form-group">' +
            ...
            '</label>' +
            '<input type="text" id="bill_to" name="bill_to" placeholder="例: 株式会社サンプル　御中" required maxlength="100">' +
          '</div>' +
          '<div class="form-group">' +
            '<label for="customer_email">' +
              'メールアドレス（任意）' +
            '</label>' +
            '<input type="email" id="customer_email" name="customer_email" placeholder="example@company.com" maxlength="100">' +
